   ...
   const calculateAmazonFees = (productData) => {
       ...
       console.log(`--- calculateAmazonFees START ---`); // Added Log
   ...
   // --- Scoring Helper Functions (Normalization) --- V34.1 (Based on V31 + New Sales/TTS)
   ...
   const normalizeBSR = (bsr) => { /* Using BSR as proxy... */ if(!bsr||bsr<=0)return 0; const divisor=20000; return Math.max(0,Math.min(1,1/(1+bsr/divisor))); }; // Still needed for BSR proxy warning
   ...
   // *** NEW: Normalize Estimated Sales Per Month (Small Effect Steps) ***
   const normalizeSalesPerMonth = (sales) => {
       ...
       else if (sales >= 250) { score = 1.0; /* console.log("Sales Score: 1.0 (>=250)"); */ }
       else if (sales >= 150) { score = 0.9; /* console.log("Sales Score: 0.9 (150-249)"); */ }
       else if (sales >= 75) { score = 0.8; /* console.log("Sales Score: 0.8 (75-149)"); */ }
       else if (sales >= 25) { score = 0.7; /* console.log("Sales Score: 0.7 (25-74)"); */ }
       else { score = 0.6; /* console.log("Sales Score: 0.6 (<25)"); */ }
       return score;
   };
   // *** NEW: Normalize Estimated Time To Sale (Days) (Small Effect Steps) ***
   const normalizeTimeToSale = (days) => {
       ...
       else if (days <= 3) { score = 0.9; /* console.log("TTS Score: 0.9 (2-3d)"); */ }
       else if (days <= 5) { score = 0.8; /* console.log("TTS Score: 0.8 (4-5d)"); */ }
       else if (days <= 8) { score = 0.7; /* console.log("TTS Score: 0.7 (6-8d)"); */ }
       else if (days <= 15) { score = 0.6; /* console.log("TTS Score: 0.6 (9-15d)"); */ }
       else if (days <= 30) { score = 0.5; /* console.log("TTS Score: 0.5 (16-30d)"); */ }
       else { score = 0.4; /* console.log("TTS Score: 0.4 (>30d)"); */ }
       return score;
   };
   
   
   // --- calculateScore Function (Version 34.1 - Based on User's V20 + All Optional Metrics Corrected) ---
   exports.calculateScore = async (req, res) => {
       console.log("V34.1 Scoring request received.");
       ...
       try {
           ...
           let estimatedFees = 0; let determinedSizeTier = 'Unknown'; let fulfillmentFee = 0; // Used for warnings
           try {
           ...
           // --- Profitability Calculation (Includes Ad Cost AND Discount/Rebate) --- *** CORRECTED ***
           console.log("--- Step 2: Profitability Calculation ---");
           const netProfit = parsedSellingPrice - estimatedFees - parsedCostPrice - parsedAdvertisingCost + parsedDiscountRebate; // Added Rebate
           const roi = (parsedCostPrice > 0) ? (netProfit / parsedCostPrice) * 100 : 0; // ROI reflects rebate
           ...
           // --- 1. Define Weights (Replaced BSR, Added TTS, Adjusted ROI, Total=111) --- *** UPDATED ***
           const weights = {
               roi: 27, // Reduced weight
               salesPerMonth: 22, // Replaced BSR weight
               timeToSale: 5, // Added new weight
               salesTrend: 10, delicacy: 8, variationsCount: 8,
               competitionCount: 8, amazonPresence: 7,
               weight: 4, dimensions: 3, seasonality: 4,
           };
           const totalDefinedWeight = 111; // New total
           console.log(` -> Total Defined Weight: ${totalDefinedWeight}`);
   
           // --- 2. Normalize Metrics ---
           const parsedBSR = parseInt(bsr); // Keep BSR for potential warnings
           ...
           const normalized = {
               ...
               salesPerMonth: normalizeSalesPerMonth(parsedSalesPerMonth), // Use new function
               timeToSale: normalizeTimeToSale(parsedTimeToSale), // Use new function
               ...
               // bsr removed from direct scoring
           };
           ...
           // --- Generate Warnings (Add warnings for new metrics) ---
           console.log("--- Step 5: Generating Warnings ---");
           // ... (Existing warnings for ROI, Comp, Weight, Dimensions, Delicacy, Amazon, Trend, Vars, Seasonality, AdCost, Hazmat, Placement) ...
           // ROI
           ...
           // BSR (Proxy Warning)
           if (parsedSalesPerMonth === null) { // Only show BSR warning if sales/month not provided
               const BSR_CRITICAL_THRESHOLD = 150000; const BSR_WARNING_THRESHOLD = 100000; if (!isNaN(parsedBSR) && parsedBSR > BSR_CRITICAL_THRESHOLD) { warnings.push({ level: 'critical', metric: 'BSR (Proxy)', message: `BSR (${parsedBSR}) critically high (>${BSR_CRITICAL_THRESHOLD}). Using BSR as proxy for sales.` }); } else if (!isNaN(parsedBSR) && parsedBSR > BSR_WARNING_THRESHOLD) { warnings.push({ level: 'warning', metric: 'BSR (Proxy)', message: `BSR (${parsedBSR}) high (${BSR_WARNING_THRESHOLD}-${BSR_CRITICAL_THRESHOLD}). Using BSR as proxy for sales.` }); }
           }
           // Competition
           ...
           // ** NEW Warnings for Sales/Month & TimeToSale **
           const SALES_LOW_WARN_THRESHOLD = 25;
           if (parsedSalesPerMonth !== null) {
               if(parsedSalesPerMonth < SALES_LOW_WARN_THRESHOLD) { warnings.push({ level: 'warning', metric: 'Est Sales/Mo', message: `Estimated Sales (${parsedSalesPerMonth}/mo) are low (< ${SALES_LOW_WARN_THRESHOLD}). May indicate slow turnover.` }); }
           } else {
                warnings.push({ level: 'info', metric: 'Est Sales/Mo', message: `Est Sales/Month not provided. Score does not factor this in.` });
           }
           const TIME_SLOW_WARN_THRESHOLD = 30; // Warn if > 30 days
           if (parsedTimeToSale !== null) {
               if (parsedTimeToSale > TIME_SLOW_WARN_THRESHOLD) { warnings.push({ level: 'warning', metric: 'Est Time to Sale', message: `Est. Time to Sale (${parsedTimeToSale} days) is long (> ${TIME_SLOW_WARN_THRESHOLD} days). Consider holding costs.` }); }
           } else {
               warnings.push({ level: 'info', metric: 'Est Time to Sale', message: `Est. Time to Sale not provided.` });
           }
           ...
           const explanation = `(AI Placeholder - V34.1 Corrected Optional Metrics) Score ${finalScore}/100...`;
